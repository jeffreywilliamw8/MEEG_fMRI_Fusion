...
PLOTS_DIR = '/home/jeffreykatab/Projects/fusion/THINGS/Encoding_Models/plots/granger_causality'
...
full_times = 1000 * meg_metadata['meg']['times']
t_plot = full_times[14:141]
n_times = len(t_plot)
...
for roi_pair in tqdm(roi_pairs):
    for cross_validate in ['cv', 'ncv']:
        for time_aggregation in ["avg", "pca", "none"]: 
            ...
            for sub in subject_list:
                data_dir = f'/home/jeffreykatabo/Projects/fusion/THINGS/Encoding_Models/results/granger_causality_analysis/phase_1/{cross_validate}/fmri_sub-{sub}'
                res_path = os.path.join(data_dir, f'{roi_pair}_{time_aggregation}.npy')
            ...
            ax.set_title(f'Granger Causality: {area_a} ↔ {area_b} | Cross-Validation: {map_dict[cross_validate]} | RDM Time Aggregation: {map_dict[time_aggregation]}', fontweight='bold', pad=50) 
            ...
            plt.tight_layout()
            file_name = f'gc_1_{roi_pair}_{cross_validate}_{time_aggregation}_z.png'
