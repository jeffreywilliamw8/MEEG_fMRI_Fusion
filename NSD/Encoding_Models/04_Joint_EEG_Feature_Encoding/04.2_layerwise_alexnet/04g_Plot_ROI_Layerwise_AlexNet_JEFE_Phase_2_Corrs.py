...
# Define the structural groupings per individual ROI
roi_groups = {
    'V1': ['V1v', 'V1d'],
    'V2': ['V2v', 'V2d'],
    'V3': ['V3v', 'V3d'],
...
area_labels = ['V1', 'V2', 'V3', 'hV4', 'ventral', 'ventral']
...
PLOTS_DIR = '/scratch/jeffreykatab/Code/Encoding_Models/NSD/plots/jefe_phase_2/roi/layerwise_alexnet'
...
for idx, area in enumerate(area_labels):
    ...
    for l_idx, layer in enumerate(layer_keys):
        ...
        print(f"ROI:{area_labels[idx]}; Layer:{layer}; Onset: {onsets[0]} ms; Mean score: {np.mean(m_group[51:])}") # mean score computed across starting from stimulus onset (time index 51)
        ...
        ax.plot(times, m_group, color=color, lw=3, label=legend_text, zorder=3)
    ...
    ax.set_xlim(-100, 600)
    ax.set_ylim(bottom=-0.01, top=0.47)
...
plt.suptitle('', fontweight='bold', fontsize=18, y=1.0)

plt.tight_layout()
save_path = os.path.join(PLOTS_DIR, 'layerwise_alexnet_jefe.svg')
