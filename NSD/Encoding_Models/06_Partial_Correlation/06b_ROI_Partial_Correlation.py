...
combined_weights_file = os.path.join(f'/scratch/jeffreykatab/Projects/fusion/NSD/Encoding_Models/results/regression_weights/jefe_phase_2/roi/dnn_type-both/subject-{args.subject}', f'{args.roi}_{args.hemisphere}_cv_split-odd.npy')
...
fmri_test = np.expand_dims(fmri_test, 2) # additional dimension used later for vertex-wise partial correlation computations
...
print("Shape of the stimulus features (Vision DNN, LLM): {}, {}".format(vision_features.shape, language_features.shape))
print("Shape of the combined stimulus features: {}".format(combined_features.shape))
# =============================================================================
...
results = {
    "total_correlation": np.zeros((n_time, n_vertices)),      # Combined baseline
...
for t in tqdm(range(n_time)):
    ...
    for vertex in range(n_vertices):
        ...
        # 3. Total correlation, variance explained by vision and language combined
        total_correlation = np.corrcoef(y.flatten(), x_comb.flatten())[1,0]

        # 4. Store results
        results["total_correlation"][t, vertex] = total_correlation
        results["vision_partial_correlation"][t, vertex] = vision_partial_correlation
        results["language_partial_correlation"][t, vertex] = language_partial_correlation
...
# Saving the metrics
# =============================================================================   
save_dir = f'/scratch/jeffreykatab/Projects/fusion/NSD/Encoding_Models/results/partial_correlation/subject-{args.subject}'
