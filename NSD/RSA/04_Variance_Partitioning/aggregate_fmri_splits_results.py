...
for subject in tqdm(subject_list):
    for variance_type in variance_types:
        ...
        # f'/scratch/jeffreykatab/Projects/fusion/NSD/RSA/results/variance_partitioning/wb/subject-{args.subject}/hemisphere-{args.hemisphere}'
        path_l = f'/scratch/jeffreykatab/Projects/fusion/NSD/RSA/results/variance_partitioning/wb/subject-{subject}/hemisphere-lh'
        path_r = f'/scratch/jeffreykatab/Projects/fusion/NSD/RSA/results/variance_partitioning/wb/subject-{subject}/hemisphere-rh'
        ...
        for i in range(1,22):
            splits_path = path_r+f'/fmri_split-{i}' #iterating over the split folders
            corrs = np.load(splits_path+'.npy', allow_pickle=True).item()[variance_type]
            for j in range(len(corrs)):
        ...
        file_name_r = f'{variance_type}_right.npy'
