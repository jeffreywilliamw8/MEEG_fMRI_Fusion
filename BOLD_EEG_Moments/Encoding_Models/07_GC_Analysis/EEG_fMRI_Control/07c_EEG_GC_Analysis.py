...
def get_eeg_data(subject_number, channel_group_name):
    """
    Loads the EEG data for a given subject and channel group (Occipital (O), Temporal (T), or parietal (P)).
    
    input: subject_number (str), channel_group_name (str)
    output: 2 numpy arrays (training and testing) of shape (Stimuli, channels, time) 
    """

    eeg_path = f'/scratch/giffordale95/projects/eeg_moments/dataset/preprocessed_data/eeg/sub-01/mvnn-time/baseline_correction-01/highpass-0.01_lowpass-100/sfreq-{args.eeg_frequency:04d}/preprocessed_data.npy'
    ch_names = np.load(eeg_path, allow_pickle=True).item()["ch_names"]  # Loading the time vector from the EEG data
    indices = [i for i, name in enumerate(ch_names) if name.startswith(channel_group_name)]
